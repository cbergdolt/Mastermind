...
void Board::checkKey()
{
    ...
    int i, j, k;
    int num_of_pegs = 4;	//4 pegs in each guess
    ...
    vector <int> compMatches;
    vector <int> playerMatches;
    compMatches.push_back(99);		//store column number of matched computer pegs
    playerMatches.push_back(99);	//store column number of matched player pegs
    ...
    for (j = 0; j < num_of_pegs; j++) { // Go through each player peg
		...
		//check computer matches for already matched pegs
                for (vector<int>::iterator it = compMatches.begin(); it != compMatches.end(); ++it){
                    if (k == *it ) alreadyMatched = true;
                }
		//check player matches for already matched pegs
                for (vector<int>::iterator it2 = playerMatches.begin(); it2 != playerMatches.end(); ++it2){
                    if (j == *it2 ) alreadyMatched = true;
		}
                if (!alreadyMatched){
                    color_hint++;
                    compMatches.push_back(k);       // add current computer peg to matches
		    playerMatches.push_back(j);     // add current player peg to matches
                }
                alreadyMatched = false;	//reset alreadyMatched
            }
        }
    }
