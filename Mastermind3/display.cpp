...
void Display::update(int x, int y, Board *board) {
        ...
        //Analyze info about that click
//        string currentColor;
//        currentColor = board->checkClick(x, y);
//        if (currentColor != "noColorSelected") { //Color was selected
////            CURRENT = loadTexture(currentColor);
//            currentColor = "images/"+currentColor+".png";
//            cout << "Here " << currentColor << endl;
//            CURRENT = loadTexture(currentColor);
//        }
        if (board->checkClick(x, y) != "noColorSelected") {
        ...
        for (j=315; j<(WIDTH-50); j=j+90){ //x
            for (i=45; i<(HEIGHT-35); i=i+75) { //y
                if (x >= j && x <= j+90 && y >= i && y <= i+90) {
                    ...
                    board->grid[row][col].color=currentColor;
